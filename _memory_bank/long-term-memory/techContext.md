...
- `_docs/` contains 13 markdown files generated by Claude 4.5 -- many are redundant with each other and with `_memory_bank/`. Could be consolidated.
- `_docs/PROJECT_SUMMARY.md` references stale info (Rust 1.75, ~50MB image) -- now Rust 1.85, ~100MB image
